...
set.seed(2022)
index1 = sample(1:nrow(finalData),floor(0.85*nrow(finalData))) ##Use same split for logistic regression and RF
...
bart_table <- table(bart.pred, actual)
f_cfm(bart_table)
```
...
```{r}
library(formattable)
data_frame <- data.frame(c("Logistic Regression", "Decision Tree", "Random Forest","BART"), c(f_logistic, f_tree, f_forest,f_bart),c(round(auc(logistic_roc), 4), round(auc(tree_roc), 4), round(auc(rf_roc), 4), round(auc(bart_roc), 4)))
